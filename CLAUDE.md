...
| `scripts/` | **Owned by us.** `setup.ts` is the one-command onboarding (`bun run setup`): install → build → binary onto PATH → global-config → prompt+validate+store `GWDG_API_KEY` (always prompts even if one is set — blank keeps the current key; a pasted key is tested against GWDG `/v1/models` via `validateApiKey()` and re-prompted on failure). It chains the other two: `build-iibcode.ts` builds the standalone binary (`iibcode:build`); `install-global-config.ts` syncs `opencode.json` to `~/.config` (`setup:global-config`). |
...
| `cli/cmd/tui/util/gwdg-refresh-command.ts` | **Entirely ours:** the command handler — resolve **all** config targets → fetch catalog once → probe the union of new ids → scrape context limits → per-config: add new + refresh limits + **prune** decommissioned → write → one SIGUSR2 reload |
| `cli/cmd/tui/util/gwdg-refresh.ts` | **Entirely ours:** the refresh + docs-scrape logic. `resolveConfigTargets()` returns **both** the nearest project-local `opencode.json` (walk up via `findOpencodeJson`) **and** the global `~/.config/opencode/opencode.json` (`globalConfigPath()`, XDG path), deduplicated, so `/models-refresh` keeps every config in sync. `pruneStale()` deletes models no longer in the catalog (protecting the top-level default). `validateApiKey()` (a classified GET `/v1/models`) is reused by `scripts/setup.ts`. (`resolveConfigPath()` — single-target — is kept but unused.) |
...
- Run `/models-refresh` inside the TUI to sync (adds new GWDG models whose tool probe returns 2xx; a 4xx to the probe means no server-side tool support → skipped, and the success toast reports `Added N, removed M, skipped K without tool calling` so the count matches what `/models` then shows). It also refreshes per-model context limits from the GWDG docs. As of 2026-07-05 it is a **true sync**: it edits **both** the project-local and global `opencode.json` (when both exist) and **auto-prunes** models that vanish from the catalog (so `qwen3-coder-30b-a3b-instruct` and any other decommissioned model drop out automatically — no more manual pruning). The one exception: the model set as the top-level `model` default is never pruned even if the catalog momentarily drops it (guards against transient delisting breaking TUI startup).
